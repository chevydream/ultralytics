...
#网络结构
NetStruct_n         = "ultralytics/cfg/models/v8/myModel_yolov8n.yaml"
NetStruct_s         = "ultralytics/cfg/models/v8/myModel_yolov8s.yaml"

#预训模型
PreModel_n          = "E:/DLModel/curWeights/yolov8n.pt"
PreModel_s          = "E:/DLModel/curWeights/yolov8s.pt"
...
#模型结构|预训练模型|续训练模型
NetStruct       = NetStruct_n
PreModel        = PreModel_n
PreModel_jx     = "E:/DLRuns/detectV8/train20240508_104333/weights/best.pt"
...
def myExport(saveName, trainFlag):
    ...
    if trainFlag == 0:
        ...
        model_path = "E:/DLRuns/detectV8/{:}/weights/best.pt".format(saveName)
    ...
    # 将模型导出为 ONNX 格式(fp32)
    model.export(format="onnx", half=False, dynamic=False, opset=12)

    # 将模型导出为 OpenVINO 格式(fp16)
    model.export(format="openvino", half=True, dynamic=False, opset=12)

    # 将模型导出为 rknnopt 格式(fp16)
    model.export(format="rknn", half=True, dynamic=False, opset=12)

    # 将模型导出为 ONNX 格式(fp16)
    model_path2 = model_path.replace(".pt", "-fp16.pt", 1)
    shutil.copy2(model_path, model_path2)
    model2 = YOLO(model_path2)
    model2.export(format="onnx", half=True, dynamic=False, opset=12)
...
def myTrain(typeFlag, saveName):
    ...
    srcPath = "E:/DLRuns/detectV8/{:}/weights/best.pt".format(saveName)
